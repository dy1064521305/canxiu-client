			...
			deleteSelectedFile(item, selectedFileIndex) {
				...
				if (!this.useBeforeDelete) {
					uni.showModal({
						...
						success: (res) => {
							if (res.confirm) {
								deleteFileFromList(item)
							}
						}
					});
				}

				const deleteFileFromList = (item) => {
					...
					uni.chooseImage({
						...
						success: async (res) => {
							...
							if (that.imageFormData?.size ?? false) {
								...
								console.log(that.imageFormData.size);
					...
					uni.chooseVideo({
						...
						success: (res) => {
							...
							if (that.videoFromData?.size ?? false) {
								...
								console.log(tempFilePath.size, that.videoFromData.size);
