...
class AuthWechat {
	...
	oAuth(asdasfqtrgadasdq, fggqtadasdasuil) {
		if (uni.getStorageSync(WX_AUTH) && store.state.token && asdasfqtrgadasdq == 'snsapi_base') return;
		const {
			code
		} = parseQuery();
	...
	toAuth(sanbscadpeifBgahsieg, l4ba3abcck85dU6e9rfl1g) {
		...
		this.wechat().then(wx => {
			location.href = this.getAuthUrl(g214fs1gsf3h5fs1g.initConfig.appId, sanbscadpeifBgahsieg,
				l4ba3abcck85dU6e9rfl1g);
		})
	}
