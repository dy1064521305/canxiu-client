		...
		<view v-if="skeletonLoading" class="x-skeleton__wrapper" :class="[ startFadeOut && 'fade-out' ]"
			...
			<view v-for="(row, rowIndex) in gridRowsArr" :key="rowIndex" class="x-skeleton__wrapper__rows"
				...
				<view v-for="(column, columnIndex) in gridColumnsArr" :key="columnIndex"
					class="x-skeleton__wrapper__columns" :style="{ 
					...
					<view v-if="skeletonConfigs.headShow" class="x-skeleton__wrapper__head"
					...
					<view v-if="skeletonConfigs.textShow" class="x-skeleton__wrapper__text" :style="{
						   padding:skeletonConfigs.textPadding,
					   }">
						<view v-for="(text, textIndex) in textRowsArr" :key="textIndex"
							class="x-skeleton__wrapper__text__row" :class="[animate && 'animate']" :style="{
								width: text.width, 
								height: text.height, 
		...
		computed: {
			...
			textRowsArr() {
				...
				for (let i = 0; i < this.skeletonConfigs.textRows; i++) {
					...
					let item = {},
						// 需要预防超出数组边界的情况
						rowWidth = this.isArray(textWidth) ? (textWidth[i] || (i === gridRows - 1 ? '70%' : '100%')) :
						i === gridRows - 1 ? '70%' : textWidth,
