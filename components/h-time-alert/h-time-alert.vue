	...
	<view :class="{ appintAlertMask: true, aMShow: isShow }" @click="_maskClose">
		<view :class="{ appintAlertContent: true, aCShow: isShow }" @click.stop="_stopFunc">
			...
			<view class="alertTimebox">
				<view class="left_box">
					<view v-if="item.timeArr.length > 0" @click="_changeDay(index)" :class="{ active: item.checked }"
						v-for="(item, index) in timeList" :key="item.dateStr">
						{{ item.name }}
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			_toLocalTime(v, type = 0) {
				...
				v = type === 0 ? `${this._addZero(h)}:${this._addZero(m)}` :
					`${this._addZero(h)}:${this._addZero(m)}:${this._addZero(s)}`;
				return v;
			},
